...
class BrowserLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.browser_widget = CefBrowser(start_url="http://localhost:5500")
...
class CefBrowser(Widget):
    ...
    def on_touch_down(self, touch, *kwargs):
        ...
        if touch.is_double_tap:
            ...
            self.browser.SendMouseClickEvent(
                touch.x, y, cef.MOUSEBUTTON_RIGHT, mouseUp=False, clickCount=1
            )
            self.browser.SendMouseClickEvent(
                touch.x, y, cef.MOUSEBUTTON_RIGHT, mouseUp=True, clickCount=1
            )
        else:
            self.browser.SendMouseClickEvent(
                touch.x, y, cef.MOUSEBUTTON_LEFT, mouseUp=False, clickCount=1
            )
    ...
    def on_touch_up(self, touch, *kwargs):
        ...
        y = self.height - touch.pos[1]
        self.browser.SendMouseClickEvent(
            touch.x, y, cef.MOUSEBUTTON_LEFT, mouseUp=True, clickCount=1
        )
            ...
            if self.is_inside_web_view(touch.x, y):
                if self.is_drag_leave:
                    ...
                    self.browser.DragTargetDragEnter(
                        self.drag_data, touch.x, y, cef.DRAG_OPERATION_EVERY
                    )
        ...
        if self.is_drag:
            if self.drag_icon:
                ...
                self.drag_icon = Rectangle(
                    texture=texture, pos=self.flip_pos_vertical(x, y), size=(width, height)
                )
...
class ClientHandler:
    ...
    def _fix_select_boxes(self, frame):
        ...
        resources_dir = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "kivy-select-boxes"
        )
